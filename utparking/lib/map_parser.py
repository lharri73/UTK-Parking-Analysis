...
class Parser:
    ...
    def __init__(
        ...
        if isinstance(data, list):
            ...
            self.buildings = [
                Building(b["name"], b["lat"], b["lon"], b["cat"],
                         b.get('code', None)) for b in data["buildings"]
            ]
            self.parking = [
                Building(b["name"], b["lat"], b["lon"], b["cat"])
        ...
        self.distance_matrix = np.empty(
            (2, len(self.buildings), len(self.parking)))
    ...
    def gen_matrix(self, origin, destination):
        ...
        for i, chunk in enumerate(chunks_origin):
            for j, dest in enumerate(dest_origin):
                with open(f"{self.results_folder}/{self.results_counter}.json",
                          "r") as f:
                    result = json.load(f)
                self.results_counter += 1
                ...
                for k, row in enumerate(result["rows"]):
                    for l, element in enumerate(row["elements"]):
                        if element["status"] == "OK":
                            matrix[0, i * num_items + k, j * num_items +
                                   l] = element["distance"]["value"]
                            matrix[1, i * num_items + k, j * num_items +
                                   l] = element["duration"]["value"]
                        else:
                            matrix[:, i * num_items + k,
                                   j * num_items + l] = np.nan
        return matrix
